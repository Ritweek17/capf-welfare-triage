...
```
[Dashboard (single responsive React app,
role-conditional rendering for all three roles)]
                 | HTTPS/JSON
                 v
...
- **Dashboard** is one responsive web app serving Commander, Welfare Officer, and Personnel views through role-conditional rendering. The Personnel view includes self-check-in and works on a phone-sized screen to satisfy the PS's mobile-based requirement.
...
- **Dashboard** reads the `role` claim out of the JWT issued at login and conditionally renders the Commander view (aggregates only), Welfare Officer view (flagged individuals + reasons), or Personnel view (self-check-in). Same components, same bundle, different data and actions the API is willing to return for that role.

## Data Flow: Personnel Check-In → Alert

1. **Personnel submits a check-in** (mood scale, optional note, optional structured questions) via the responsive dashboard's Personnel view → `POST /checkins` on the Backend, authenticated as that person.
2. **Backend writes the check-in** to the database, tagged with `person_id` and `timestamp`. It returns `{"status": "received"}` to the Personnel view — **no score, ever, in this response** (Decision 2).
...
| Step | Gate |
|---|---|
| Personnel self-check-in submission | Person can only write their own check-in (JWT `person_id` must match) |
| Personnel self-check-in response | Hardcoded to never include a score field, regardless of role |
...
## Why the Dashboard Is One Responsive App

Same layout shell, same design system, same login flow — the only thing that changes is which API endpoints the logged-in role is allowed to call and which components render with that data. Commander, Welfare Officer, and Personnel are all served by the same responsive dashboard app, including the Personnel self-check-in view.

## Why Responsive Design Satisfies the Mobile Requirement

The dashboard's responsive Personnel view satisfies the PS's "mobile-based" requirement through a phone-friendly web experience, without a separate mobile app or folder.
